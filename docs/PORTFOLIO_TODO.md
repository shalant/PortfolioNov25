...
- **Prerendering (2026-09-05):** the deeper limitation is now addressed — `tools/prerender/prerender.mjs` runs as a build step (`.github/workflows/publish-gh-pages.yml`) after `dotnet publish`: serves the published build locally, drives headless Chromium to every known route (all `/webdesign/{slug}` case studies, `/services`, `/consulting`, `/blog`, `/blog/archive`, `/`), waits for Blazor to finish rendering, and writes the resulting DOM back as that route's static `index.html`. Blazor still boots and takes over for real visitors, same swap-not-merge behavior as the hand-authored homepage hero preview — this just generates the same idea for every route automatically instead of hand-maintaining one. Verified locally against a real `dotnet publish` output before wiring into CI. Bonus: routes that relied on the `404.html` SPA-redirect trick for direct navigation now serve a real static file instead, so that trick is only load-bearing for genuinely unlisted paths.
- **Follow-up (not yet due):** check GA4 → Acquisition → Traffic acquisition in ~6 weeks (mid-to-late Oct 2026) for any new referral traffic from AI-assistant domains (chat.openai.com, perplexity.ai, copilot.microsoft.com, gemini.google.com) or an uptick in unexplained direct traffic, now that llms.txt + prerendering are live. Also check Search Console for AI Overview impressions/clicks. Traffic is currently tiny, so don't expect a clean signal — write up whatever the actual delta is, including if it's zero.
- **Real indexing/backlink pass (2026-09-05):** live check found the site essentially unindexed (`site:dougrosenbergdev.com` returns nothing) and its only backlink (GitHub profile) is `rel="nofollow"`. Fixed the broken `og:image`/schema image, the weak `<title>`/`<PageTitle>` tags, gave each blog post a real crawlable `/blog/{slug}` route with per-post meta tags (previously modal-only, invisible to any crawler), added IndexNow, and opened a real backlink PR on the ArborKin repo. Full detail in `docs/TASKS.md`'s "SEO/GEO follow-up" entry. Still open: Search Console re-indexing request, and a branding decision on whether the Facebook music page belongs in schema `sameAs`.
- **Bing Webmaster Tools — done (2026-09-05).** User signed up (account creation/login isn't something Claude does — walked through it live instead), imported the site directly from the already-verified Google Search Console property (instant, no separate verification needed), and submitted `sitemap.xml` — reported success with 13 URLs discovered, matching the sitemap's actual entry count exactly. Separately verified the IndexNow ping (added earlier the same day) actually works: the key file 200s at the site root, and a manual resubmission to `api.indexnow.org` returned `HTTP/2 200` — the empty response body is IndexNow's normal success response, not a failure, confirming the silent-looking CI log from the PR #52 deploy was in fact a successful ping.
